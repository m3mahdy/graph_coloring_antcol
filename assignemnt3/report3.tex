...
The ACO algorithm's performance is evaluated against two baseline methods for comparative analysis: \textbf{Greedy} (simple constructive heuristic providing fast approximate solutions) and \textbf{Tabu Search} (the S-metaheuristic implemented in Assignment 2). Table~\ref{tab:performance_comparison} presents detailed performance statistics across all three algorithms.
...
The experimental results (Table~\ref{tab:performance_comparison}) demonstrate clear trade-offs between the three algorithmic paradigms, as illustrated in Figures \ref{fig:comparison_best}, \ref{fig:comparison_avg}, and \ref{fig:comparison_time}:
\begin{itemize}
