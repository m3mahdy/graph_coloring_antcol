...
This report implements a \textbf{population-based metaheuristic} (Ant Colony Optimization) to address the computational complexity of the GCP. ACO leverages collective intelligence of multiple agents to explore the solution space through constructive pheromone-guided building of solutions. The results are compared against two baseline algorithms: \textbf{Greedy} (simple constructive heuristic) and \textbf{Tabu Search} (single-solution metaheuristic from Assignment 2).
...
\begin{enumerate}
    ...
    \item \hspace{1em} \textbf{// Parallel Ant Construction Phase}
    \item \hspace{1em} \textbf{For} $ant = 1$ \textbf{to} $m$ \textbf{Do (in parallel)}
    \item \hspace{2em} $S_{ant} \leftarrow $ ConstructSolution($G$, $\tau$, $\alpha$, $\beta$)
    \item \hspace{2em} $k_{ant} \leftarrow $ CountColors($S_{ant}$)
    \item \hspace{1em} \textbf{End For}
    \item \hspace{1em} \textbf{// Find Iteration-Best Solution}
    \item \hspace{1em} $S_{iter} \leftarrow \argmin_{ant} k_{ant}$, $k_{iter} \leftarrow \min_{ant} k_{ant}$
    \item \hspace{1em} \textbf{If} $k_{iter} < k_{best}$ \textbf{Then}
    \item \hspace{2em} $S_{best} \leftarrow S_{iter}$, $k_{best} \leftarrow k_{iter}$, $no\_improve \leftarrow 0$
    \item \hspace{1em} \textbf{Else}
    \item \hspace{2em} $no\_improve \leftarrow no\_improve + 1$
    \item \hspace{1em} \textbf{// Pheromone Update}
    \item \hspace{1em} $\tau[v][c] \leftarrow (1-\rho) \cdot \tau[v][c]$ for all $v, c$ \quad // Evaporation
    \item \hspace{1em} \textbf{For each} $(v, c) \in S_{iter}$ \textbf{Do}
    \item \hspace{2em} $\tau[v][c] \leftarrow \tau[v][c] + Q / k_{iter}$ \quad // Reinforcement
    \item \hspace{1em} \textbf{If} $no\_improve \geq p \cdot T$ \textbf{Then} \textbf{Break} \quad // Early stopping
    \item \textbf{End For}
...
\begin{enumerate}
    ...
    \item \hspace{1em} $neighbor\_colors \leftarrow \{S[u] : u \in N(v), u \in S\}$
    \item \hspace{1em} $valid\_colors \leftarrow \{c : c \notin neighbor\_colors\}$ \quad // No conflicts
    \item \hspace{1em} \textbf{If} $valid\_colors = \emptyset$ \textbf{Then} \quad // Expand color set
    \item \hspace{2em} Add new color to $\tau$ and $valid\_colors$
    \item \hspace{1em} \textbf{For each} $c \in valid\_colors$ \textbf{Do}
    \item \hspace{2em} $\eta[c] \leftarrow 2.0$ if $c \in used\_colors$ else $1.0$ \quad // Heuristic
    \item \hspace{2em} $score[c] \leftarrow [\tau[v][c]]^{\alpha} \cdot [\eta[c]]^{\beta}$
    \item \hspace{1em} $P[c] \leftarrow score[c] / \sum_{c'} score[c']$ \quad // Normalize probabilities
    \item \hspace{1em} $c_{chosen} \leftarrow $ SampleColor($valid\_colors$, $P$) \quad // Probabilistic selection
    \item \hspace{1em} $S[v] \leftarrow c_{chosen}$, $used\_colors \leftarrow used\_colors \cup \{c_{chosen}\}$
    \item \textbf{End For}
    \item \textbf{Return} $S$
\end{enumerate}
...
The GCP is $\text{NP}$-hard, meaning that finding the optimal chromatic number for large graphs is computationally infeasible using exact methods. While single-solution metaheuristics like Tabu Search have proven effective, they suffer from several limitations: dependency on initial solution quality, vulnerability to local optima, and limited exploration of the solution space. \textbf{Ant Colony Optimization} is chosen as a population-based alternative because:

\begin{itemize}
    \item \textbf{Constructive Approach:} ACO builds solutions from scratch rather than improving existing ones, eliminating dependency on initial solution quality and avoiding local optima traps inherent in improvement-based methods.
    
    \item \textbf{Collective Intelligence:} Multiple ants explore the solution space in parallel, enabling simultaneous investigation of diverse solution construction paths and more robust exploration than single-solution methods.
...
\end{itemize}
All experiments were conducted on a local machine running macOS with an Intel(R) Core(TM) i7 processor (3.40 GHz) and 32.0 GB of RAM. The implementation is available in the \texttt{code/} directory with modular structure for reproducibility.
...
\begin{itemize}
    ...
    \item \textbf{Optimization Duration:} Approximately 130 hours of computation
...
The extensive hyperparameter optimization process (Figure~\ref{fig:optuna_history}) consumed approximately 130 hours of computation across 40 trials, ultimately converging on trial 19 with 200 colors as the best configuration. This optimization journey revealed critical insights into ACO's behavior for graph coloring. The tuning process demonstrated that \textbf{high beta} (strong heuristic influence) and \textbf{low rho} (slow evaporation) were essential for performance, fundamentally emphasizing intensification over diversification—a counterintuitive finding for a metaheuristic typically valued for exploration capabilities.
...
The average performance over three runs (Figure~\ref{fig:comparison_avg}) exposes an additional scalability problem: the performance gap between ACO and Tabu Search widens dramatically as graph size increases. On dsjc250.5 (250 vertices), ACO uses approximately 55 colors versus TS's 34 colors—a 62\% increase. However, on dsjc1000.5 (1000 vertices), ACO requires 119 colors versus TS's 53 colors—a 125\% increase. This non-linear degradation indicates that ACO's population-based constructive approach struggles increasingly with larger problem instances, likely due to pheromone signal dilution across the expanding solution space. The consistency metrics further reveal that while ACO shows low standard deviation (indicating reproducible results), it reproducibly achieves poor solutions—consistency without quality provides little practical value.
...
\begin{itemize}
    ...
    \item \textbf{Improvement:} 32-59\% better than Greedy, 61-73\% better than ACO in terms of color count.
...
\begin{itemize}
    \item \textbf{Quality:} Poorest solution quality among all three methods (58-166\% deviation from BKS).
    ...
    \item \textbf{Scalability:} Performance degradation increases with graph size (167\% deviation on dsjc1000.5 vs 96\% on dsjc250.5).
\end{itemize}
