...
\caption{Comprehensive Performance Comparison Across Three Algorithmic Paradigms}
...
\subsection{Why ACO Underperforms}
...
\subsection{Graph Characteristics Impact}
As shown in Table~\ref{tab:performance_comparison} and illustrated in Figures~\ref{fig:comparison_best} and~\ref{fig:comparison_avg}:
\begin{itemize}
...
The current ACO implementation might become competitive with enhancements such as improved heuristics (degree-based), hybrid approaches using TS-quality solutions to initialize pheromones, local search post-processing, or alternative pheromone models (noder coloring order instead of node-color).
