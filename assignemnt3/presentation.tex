...
\vspace{-0.5cm}
\begin{center}
\includegraphics[width=0.25\textwidth]{figures/KSU_Logo.png}
\end{center}
\end{frame}
...
\section{Methodology: ACO P-Metaheuristic}

\begin{frame}{Why Ant Colony Optimization?}
\textbf{ACO Key Advantages:}
\begin{itemize}
    ...
    \item \textbf{Adaptive Learning:} Pheromone-based memory guides search
    \item \textbf{Balanced Exploration-Exploitation:} Natural probabilistic mechanisms
    \item \textbf{Flexibility:} Adapts to different problem structures
\end{itemize}

\vspace{0.5cm}
\textbf{Population-based Metaheuristic:}
\begin{itemize}
    \item Multiple solutions per iteration increase diversity
    \item Parallel execution leverages multi-core processors
    \item Collective experience accumulates over iterations
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item 82 ants per iteration
    \item Multi-threading
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}{ACO Algorithm Pseudocode}
\tiny
\textbf{Initialize:} pheromone matrix $\tau[v][c] \leftarrow \tau_0$ \\[0.5em]

\textbf{FOR} iteration = 1 to max\_iterations: \\
\hspace{1em} solutions $\leftarrow$ [] \\
\hspace{1em} \textbf{FOR} ant = 1 to m (parallel): \hspace{2em} \textit{\textcolor{gray}{// Ant Solution Construction}} \\
\hspace{2em} solution $\leftarrow$ \{\} \\
\hspace{2em} vertices $\leftarrow$ shuffle(V) \\
\hspace{2em} \textbf{FOR} each $v$ in vertices: \hspace{2em} \textit{\textcolor{gray}{// Sequential vertex coloring}} \\
\hspace{3em} valid\_colors $\leftarrow$ \{$c$ : no neighbor uses $c$\} \\
\hspace{3em} \textbf{IF} valid\_colors is empty: \\
\hspace{4em} valid\_colors $\leftarrow$ \{new color\} \\
\hspace{3em} $\eta[c] \leftarrow 2.0$ if $c$ used, else $1.0$ \hspace{2em} \textit{\textcolor{gray}{// Heuristic}} \\
\hspace{3em} $P(c) \leftarrow \frac{[\tau[v][c]]^{\alpha} \cdot [\eta[c]]^{\beta}}{\sum_{c'} [\tau[v][c']]^{\alpha} \cdot [\eta[c']]^{\beta}}$ \hspace{2em} \textit{\textcolor{gray}{// Probability}} \\
\hspace{3em} solution[$v$] $\leftarrow$ select $c$ with probability $P(c)$ \\
\hspace{2em} solutions.append(solution) \\
\hspace{1em} best\_solution $\leftarrow$ argmin(solutions) \hspace{2em} \textit{\textcolor{gray}{// Select iteration best}} \\
\hspace{1em} \textbf{Evaporate:} $\tau[v][c] \leftarrow (1-\rho) \cdot \tau[v][c]$ for all $v, c$ \\
\hspace{1em} \textbf{Deposit:} $\tau[v][c] \leftarrow \tau[v][c] + \frac{Q}{\text{num\_colors}}$ for $(v,c)$ in best\_solution \\
\hspace{1em} \textbf{IF} no improvement for patience $\times$ max\_iterations: \\
...
\begin{itemize}
    ...
    \item Random starting nodes for diversity
    \item Random visitation order
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
    \item Pheromone reinforcement on best solutions
    \item Heuristic preference for used colors ($\eta=2.0$)
    \item High $\alpha$ increases exploitation
    \item High $\beta$ increases intensification
\end{itemize}
...
\begin{itemize}
    ...
    \item Probabilistic (not greedy) selection
    \item Random node ordering
    \item Multiple parallel ants
    \item Low $\rho$ promotes exploration
\end{itemize}
\end{columns}
...
\begin{itemize}
    ...
    \item \textbf{Trials:} 40 independent trials
...
\vspace{0.3cm}
\textbf{Key Findings:} High $\beta$ (heuristic) and low $\rho$ (evaporation) \\
$\rightarrow$ Intensification dominates over diversification
\end{frame}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\textwidth]{figures/slice.png}
...
\begin{figure}
    ...
    \caption{Trial 19 coloring of gc\_500\_9 - Valid conflict-free solution}
\end{figure}
\end{frame}
...
\begin{frame}{Three-Way Performance Comparison}
\begin{table}
\centering
\tiny
\begin{tabular}{llcccccc}
\toprule
\textbf{Instance} & \textbf{Algorithm} & \textbf{Best} & \textbf{Avg.} & \textbf{Std.} & \textbf{Time (s)} & \textbf{Dev. (\%)} \\
\midrule
...
\textbf{Strength:} Extremely fast (< 1 second) \\
...
\textbf{Strength:} Consistent, no initial dependency \\
\textbf{Weakness:} Poorest quality (58-167\% deviation), comparable time to TS \\
\textbf{Current Status:} Not recommended in present form
\end{block}
\end{frame}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Large color sets (50-200 colors) dilute pheromone signals
        \item TS's tabu list prevents specific moves without dilution
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Greedy:} Fast but moderate quality
    \item \textbf{Tabu Search:} Excellent quality, high cost
    \item \textbf{ACO:} Poor quality despite comparable cost to TS
\end{itemize}
\end{block}

\begin{block}{Paradigm Suitability}
\begin{itemize}
    \item \textbf{Population-based $\neq$ automatically better}
    \item Improvement-based (TS) benefits from feasible starting solutions
    \item Constructive methods (ACO) must learn from scratch
    \item Problem structure determines paradigm suitability
\end{itemize}
\end{block}

\begin{block}{Tuning Insights}
High $\beta$ and low $\rho$ essential → \textbf{Intensification > Diversification} \\
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Constructive ACO with dynamic pheromone matrix
        \item 82 parallel ants using multi-threading
    \end{itemize}
    
    \item \textbf{Comprehensive Tuning:}
    \begin{itemize}
        \item Optuna framework: 40 trials, 6 parallel workers, 46 hours
        \item 15-color improvement (215 → 200 colors)
    \end{itemize}
    
    \item \textbf{Empirical Comparison:}
    \begin{itemize}
        \item Three-way analysis: Greedy vs TS vs ACO
        \item DIMACS benchmarks (250-1000 vertices)
        \item Reveals constructive vs improvement-based trade-offs
    \end{itemize}
    
    \item \textbf{Insights:}
    \begin{itemize}
        \item Population-based methods not inherently superior
        \item For GCP: heuristics and search time > pheromone dynamics
        \item No universally best algorithm - problem-dependent
    \end{itemize}
\end{enumerate}
\end{frame}
...
\begin{itemize}
    \item Improved heuristics (degree-based, conflict-prediction)
    \item Hybrid approaches (TS-quality initialization for pheromones)
    \item Local search post-processing
    \item Alternative pheromone models (edge-color instead of node-color)
...
\begin{itemize}
    \item Investigation of why pheromone learning fails for GCP
    \item Comparison with other population-based methods (GA, PSO)
    \item Problem structure analysis for metaheuristic selection
\end{itemize}
\end{frame}
